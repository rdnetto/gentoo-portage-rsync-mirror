...
# $Header: /var/cvsroot/gentoo-x86/sys-libs/libcxx/libcxx-0.0_p20140322.ebuild,v 1.3 2015/01/17 19:10:41 blueness Exp $
...
IUSE="elibc_glibc +libcxxrt libcxxabi static-libs test"

RDEPEND="libcxxrt? ( >=sys-libs/libcxxrt-0.0_p20130725[static-libs?,${MULTILIB_USEDEP}] )
		libcxxabi? ( sys-libs/libcxxabi )
		|| ( sys-libs/libcxxrt sys-libs/libcxxabi >=sys-devel/gcc-4.7[cxx] ) "
DEPEND="${RDEPEND}
...
pkg_setup() {
	if ! use libcxxrt && ! use libcxxabi ; then
...
src_configure() {
	...
	if use libcxxrt ; then
		...
		LIBS="-lc++abi ${LIBS}"
		cp "${EPREFIX}/usr/include/libcxxrt/cxxabi.h" "${S}/include"
	else
...
gen_static_ldscript() {
	...
	# TODO: Generate a libc++.a ldscript when building against libsupc++, libc++abi
}
